...
**Description:** Returns available vehicle brands for the driver to choose when registering a vehicle. Supports cursor pagination, filters, text search, and sorting.  
...
|-----------|-----------------|--------------------------|
| `name`    | `eq`, `contains`| `name[contains]=toyota`  |
...
|--------------|---------|-------------------------------------------------|
| `items`      | array   | Array of vehicle brand objects                  |
...
- Called by `driversFacade.listVehicleBrands(params)` via `useVehicleBrands` query
- Cache with TanStack Query using `vehicleBrandsKeys.list(params)`
- After the driver selects a brand, use `GET /v1/drivers/vehicle-models/brand/{brandId}` to load models
