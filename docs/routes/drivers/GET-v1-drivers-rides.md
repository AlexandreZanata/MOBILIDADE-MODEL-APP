...
**Tag:** Drivers  
**Summary:** List driver ride history  
**Description:** Returns the authenticated driver's ride history with cursor pagination, filters, text search, and sorting.  
...
- Called by `driversFacade.listRides(params)` via `useDriverRides` query
- Cache with TanStack Query using `driverRidesKeys.list(params)`
