...
**Description:** Uploads the driver's profile photo.  
...
- Called by `driversFacade.uploadProfilePhoto(file)` via `useUploadProfilePhoto` mutation
- Use `FormData` to construct the multipart request — never send as JSON
- On `onSuccess`, invalidate `driverProfileKeys.me()` to refresh the cached profile
