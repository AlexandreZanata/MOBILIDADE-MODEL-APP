...
**Description:** Returns current rate limit information for the authenticated user.  
...
- Called by `routingFacade.getRateLimit()` via `useRoutingRateLimit` query
- Cache with TanStack Query using `routingKeys.rateLimit()`
- Use `remaining` and `resetInSeconds` to display a rate limit warning before the user hits a `429`
