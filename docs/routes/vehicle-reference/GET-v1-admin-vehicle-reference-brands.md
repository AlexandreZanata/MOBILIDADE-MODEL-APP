...
**Description:** Returns vehicle brands with cursor pagination, filters, text search, and sorting.  
...
|--------------|---------|-----------------------------------------------------------------|
| `items`      | array   | Array of vehicle brand objects for the current page             |
...
- Called by `vehicleReferenceFacade.listBrands(params)` via `useAdminVehicleBrands` query
- Cache with TanStack Query using `vehicleBrandsAdminKeys.list(params)`
